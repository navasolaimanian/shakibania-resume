  ...
  return (
    // <Li passHref legacyBehavior>
      <div className="shadow-4xl md:hover:shadow-3xl cursor-pointer grid self-center sm:grid-cols-3 mb-4 md:mb-8 w-full md:w-[90%] lg:w-[80%] xl:w-[85%] 2xl:w-[75%] 3xl:w-[70%] bg-[#F4F7FA] sm:bg-white p-5 lg:p-4 rounded-lg lg:transition lg:ease-in-out lg:delay-150 lg:hover:-translate-y-1 lg:hover:scale-[1.01]">
        ...
        <div className="sm:col-span-2 flex flex-col justify-between sm:ml-2 md:ml-10">
          <div>
            <div className="flex flex-col sm:flex-col-reverse mt-2">
              <p className={`text-xs ${!props.author ? "text-[#E3898B]" : "text-[#C26E4B]"} mt-2 sm:mt-1 inline py-1 font-bold`}>{props.data.journal}</p>
              <Link href={props.data.url}>
                <h1 className="font-bold text-sm md:text-base xl:text-lg 2xl:text-xl text-[#7C7D81]">{props.data.title}</h1>
              </Link>
            </div>
            {props.author && <div className="text-[#A8AFB4] text-[9px] md:text-xs mt-2 md:mt-6">
          ...
          <div className="[&>*]:mt-2 md:[&>*]:mt-4 flex flex-wrap [&>*]:mr-2 mb-0 sm:mb-1.5">
            {props.data.bibtex &&
              <button onClick={() => {
